...
nodedht.prototype.socketserverinit = function() {
	...
	this.socketserver.on("message", (msg, rinfo) => {
		...
		if (message.y && message.y.toString() == "q") {
			...
			switch (message.q.toString()) {
				case "ping":
					query = {
						...
						r: {
							id: Buffer.concat([
								message.a.id.slice(0, 10),
								this.nid.slice(10, 20)
							])
						}
					};
				...
				case "get_peers": //get infohash
					...
					fs.appendFile(
						"info.txt",
						"magnet:?xt=urn:btih:" + infohash_value + "\r\n",
						err => {
							if (err) throw err;
						}
					);
			...
			if (message.r.nodes) {
				for (
					let i = 0, len = message.r.nodes.length;
					i < len / 26;
					i++
				) {
					let nid = message.r.nodes.slice(0 + 26 * i, 20 + 26 * i);
					let naddress =
						message.r.nodes[20 + 26 * i] +
						"." +
						message.r.nodes[21 + 26 * i] +
						"." +
						message.r.nodes[22 + 26 * i] +
						"." +
						message.r.nodes[23 + 26 * i];
					let nport = message.r.nodes.readUInt16BE(24 + 26 * i);
					if (
						nid != this.nid &&
						!this.table.has(nid) &&
						this.tablenum < TABLEMAX &&
						nport > 0 &&
						nport < 65536
					) {
...
let client = new nodedht(6880);
client.socketserverinit();

setInterval(() => {
	for (let i = PUBLICNODES.length - 1; i >= 0; i--) {
		client.sendmessage(
			{
